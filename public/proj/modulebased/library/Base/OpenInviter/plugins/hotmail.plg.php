...
class hotmail extends openinviter_base
	...
	public function getMyContacts()
		...
		$contacts=array();
		if (preg_match_all("#0\,\'(.+)\'\,0#U",$res,$names)) 
			if(!empty($names[1]))
				{
				if (!empty($names[1][0])) unset($names[1][0]);
				foreach($names[1] as $n=>$name) if (strpos($name,',')) unset($names[1][$n]);
				}		
		if (preg_match_all("#\'\,\[\'(.+)\@(.+)\'#U",$res,$matches))
			...
			foreach($matches[1] as $key=>$value)
				{
				if (!empty($names[1][$key])) $contacts["{$value}@{$matches[2][$key]}"]=array("first_name"=>$names[1][$key],"email_1"=>"{$value}@{$matches[2][$key]}");
				elseif (!empty($matches[2][$key])) $contacts["{$value}@{$matches[2][$key]}"]=array("first_name"=>$value,"email_1"=>"{$value}@{$matches[2][$key]}");
				}
			}												
