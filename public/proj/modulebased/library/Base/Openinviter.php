...
class Base_Openinviter
{
	...
	public function __construct()
		...
		require_once($this->basePath."/Plugin/openinverter/_base.php");
	...
	public function startPlugin($plugin_name,$getPlugins=false)
		...
		elseif ($this->settings['hosted'])
			{
			if (!file_exists($this->basePath."/Plugin/openinverter/_hosted.plg.php")) $this->internalError="Invalid service provider";
			else
				{
				if (!class_exists('_hosted')) require_once($this->basePath."/Plugin/openinverter/_hosted.plg.php");
				if ($getPlugins)
					{
		...
		elseif (file_exists($this->basePath."/Plugin/openinverter/{$plugin_name}.plg.php"))
			{
			$ok=true;
			if (!class_exists($plugin_name)) require_once($this->basePath."/Plugin/openinverter/{$plugin_name}.plg.php");
	...
	public function getPlugins($update=false,$required_details=false)
		...
		if (!$valid_cache)
			{
			$array_file=array();
			$temp=glob($this->basePath."/Plugin/openinverter/*.plg.php");
